...
<head>
    ...
    <title>Tambah Peralatan ICT Baru</title>
...
<body class="bg-gray-100 p-6">
    ...
    @section('content')
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 py-6"> {{-- Container for the content --}}
            ...
            <h2 class="text-2xl font-bold mb-6 text-gray-800">Tambah Peralatan ICT Baru</h2> {{-- Title --}}
            ...
            {{-- Assuming equipment creation is handled by a standard controller action --}}
            <form action="{{ route('equipment.store') }}" method="POST">
                ...
                <div class="card">
                    ...
                    <div class="form-group">
                        ...
                        {{-- Initial status is likely 'available' when adding new equipment --}}
                        <select name="status" id="status" class="form-control" required>
                            <option value="available" {{ old('status', 'available') == 'available' ? 'selected' : '' }}>
                                Tersedia (Available)</option>
                            {{-- Other statuses might be selectable, but 'available' is most common for new --}}
                            <option value="under_maintenance" {{ old('status') == 'under_maintenance' ? 'selected' : '' }}>
                                Dalam Penyelenggaraan (Under Maintenance)</option>
                            {{-- 'on_loan', 'disposed', 'lost', 'damaged' are typically set via transactions or updates --}}
                        </select>
                        @error('status')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
