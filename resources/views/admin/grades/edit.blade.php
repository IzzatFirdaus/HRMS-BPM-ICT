...
<head>
    ...
    <title>{{ __('Edit Grade') }}: {{ $grade->name ?? 'N/A' }}</title>
...
<body class="bg-gray-100 p-6">
    ...
    @section('content')
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 py-6">
            ...
            {{-- Grade Editing Form --}}
            {{-- This form submits to the 'admin.grades.update' route, passing the $grade model, using POST method with PUT method spoofing --}}
            {{-- Assuming grade editing is handled by a standard controller action, not Livewire --}}
            <form action="{{ route('admin.grades.update', $grade) }}" method="POST">
                ...
                <div class="border border-gray-300 rounded-lg p-6 mb-6 bg-white shadow-md">
                    ...
                    <div class="mb-4">
                        ...
                        {{-- Value is populated using old('name') with a fallback to the existing $grade->name --}}
                        <input type="text" name="name" id="name"
                            ...
                            required value="{{ old('name', $grade->name) }}" {{-- Required field, repopulates value on error or uses existing data --}}
                    ...
                    <div class="mb-4">
                        ...
                        {{-- Value is populated using old('code') with a fallback to the existing $grade->code --}}
                        <input type="text" name="code" id="code"
                            ...
                            required value="{{ old('code', $grade->code) }}" {{-- Required field, repopulates value on error or uses existing data --}}
                    ...
                    <div class="mb-4">
                        ...
                        {{-- Replaced custom 'form-control' styling with Tailwind classes --}}
                        {{-- Value is populated using old('description') with a fallback to the existing $grade->description --}}
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="{{ __('Enter grade description') }}">{{ old('description', $grade->description) }}</textarea> {{-- Added placeholder, translated, repopulates value on error or uses existing data --}}
                        @error('description')
                    ...
                    <div class="mb-4">
                        <div class="flex items-center"> {{-- Use flex for checkbox and label alignment --}}
                            ...
                            {{-- Checkbox state is populated using old('requires_approval') with a fallback to the existing $grade->requires_approval --}}
                ...
                <div class="flex justify-center mt-6"> {{-- Center the button horizontally --}}
                    ...
                    <button type="submit"
                        ...
                        {{ __('Update Grade') }} {{-- Translated button text --}}
                    </button>
