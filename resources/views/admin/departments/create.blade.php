...
<head>
    ...
    <title>{{ __('Add New Department/Unit') }}</title> {{-- Translate title --}}
    {{-- Link Tailwind CSS via CDN --}}
    <script src="https://cdn.tailwindcss.com"></script>
    {{-- Removed the inline <style> block and custom CSS classes --}}
    {{-- Add any necessary custom CSS if Tailwind alone is insufficient (e.g., specific components not covered by utilities) --}}
</head>
...
<body class="bg-gray-100 p-6"> {{-- Applied Tailwind background and padding to body --}}
    ...
    @section('content')
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 py-6">
            ...
            {{-- Assuming department creation is handled by a standard controller action --}}
                ...
                <div class="border border-gray-300 rounded-lg p-6 mb-6 bg-white shadow-md">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            {{-- Added Tailwind validation styling --}} required value="{{ old('name') }}">
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="code" id="code"
                            ...
                            {{-- Added Tailwind validation styling --}} value="{{ old('code') }}">
                        @error('code')
                            {{-- Replaced custom text-danger span with Tailwind class --}}
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea> {{-- Added Tailwind validation styling --}}
                        @error('description')
                            {{-- Replaced custom text-danger span with Tailwind class --}}
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
