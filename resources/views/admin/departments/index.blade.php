...
<head>
    ...
    <title>Senarai Bahagian/Unit</title>
...
<body class="bg-gray-100 p-6">
    ...
    @section('content')
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-6"> {{-- Converted container to Tailwind --}}
            <h2 class="text-2xl font-bold mb-6 text-gray-800">Senarai Bahagian/Unit</h2> {{-- Converted h2 --}}

            {{-- Button to add new department --}}
            {{-- You might wrap this in an @can('create', App\Models\Department::class) --}}
            <a href="{{ route('admin.departments.create') }}" class="btn btn-primary mb-4"> {{-- Assuming admin.departments.create route --}}
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                </svg>
                Tambah Bahagian/Unit Baru
            </a>
            {{-- @endcan --}}
            ...
            @if ($departments->isEmpty())
                {{-- Assuming $departments is passed from the controller --}}
                <p class="text-gray-600">Tiada bahagian/unit ditemui.</p> {{-- Message if no departments --}}
            @else
                            ...
                            @foreach ($departments as $department)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border-b">
                                        ...
                                        {{-- Assuming a route named 'admin.departments.destroy' exists --}}
                                        {{-- This would typically be a form submission or Livewire action --}}
                                        {{-- <form action="{{ route('admin.departments.destroy', $department) }}" method="POST" onsubmit="return confirm('Are you sure?')" class="inline ml-4">
                                             @csrf
                                             @method('DELETE')
