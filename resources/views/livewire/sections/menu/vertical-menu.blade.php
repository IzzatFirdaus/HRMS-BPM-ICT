...
if ($user) {
    // Get the user's role names using the role package (e.g., Spatie/laravel-permission)
        ...
        if ($userRoles->contains('Admin')) {
            ...
            // If user has other roles, use the first one found as the primary role for checks
    ...
    // Ensure $menuData is available and has a menu property for looping
...
elseif (isset($menu->submenu)) {
    ...
    foreach ($slugsToCheck as $slug) {
        ...
        if ($currentRouteName && Str::startsWith($currentRouteName, $slug)) {
            $activeClass = 'active open'; // Mark as active and open
...
// based on the active state of its children, if the slug check above isn't sufficient.
                        ...
                        <li class="menu-item {{ $activeClass }}">
                            ...
                            @isset($menu->submenu)
                                {{-- Include the submenu partial, explicitly passing $menu->submenu, $role, and $user --}}
