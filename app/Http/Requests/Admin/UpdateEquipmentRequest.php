...
class UpdateEquipmentRequest extends FormRequest
{
  ...
  public function authorize(): bool
  {
    ...
    // Note: For update, the policy check should pass the Equipment model instance
    // return Auth::user()->can('update', $this->route('equipment'));
    return true;
  }
  ...
  {
    // Get the equipment ID from the route parameters to ignore it in unique checks
    $equipmentId = $this->route('equipment')->id ?? null;

    // These rules were previously in the update() method of EquipmentController
    return [
      // Tag ID should be unique, ignoring the current equipment's tag ID
      'tag_id' => ['required', 'string', 'max:50', Rule::unique('equipment', 'tag_id')->ignore($equipmentId)],
